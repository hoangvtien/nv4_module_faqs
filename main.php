...
function nv_FixWeight($catid)
{
    ...
    $sql = "SELECT id FROM " . NV_PREFIXLANG . "_" . $module_data . "_rows WHERE catid=" . $catid . " ORDER BY weight ASC";
    $result = $db->query($sql);
    $weight = 0;
...
if ($nv_Request->isset_request('add', 'get') or $nv_Request->isset_request('edit', 'get')) {
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        if (defined('IS_ADD')) {
            $sql = "SELECT COUNT(*) FROM " . NV_PREFIXLANG . "_" . $module_data . "_rows WHERE alias=" . $db->quote($alias);
            $result = $db->query($sql);
            $is_exists = $result->fetchColumn();
        } else {
            $sql = "SELECT COUNT(*) FROM " . NV_PREFIXLANG . "_" . $module_data . "_rows WHERE id!=" . $id . " AND alias=" . $db->quote($alias);
            $result = $db->query($sql);
            $is_exists = $result->fetchColumn();
        }
        ...
        if (empty($array['title'])) {
            ...
            if (defined('IS_EDIT')) {
                if ($array['catid'] != $row['catid']) {
                    $sql = "SELECT MAX(weight) AS new_weight FROM " . NV_PREFIXLANG . "_" . $module_data . "_rows WHERE catid=" . $array['catid'];
                    $result = $db->query($sql);
...
if ($nv_Request->isset_request('changeweight', 'post')) {
    ...
    $query = "SELECT catid FROM " . NV_PREFIXLANG . "_" . $module_data . "_rows WHERE id=" . $id;
    $result = $db->query($query);
    $numrows = $result->rowCount();
    if ($numrows != 1) {
        die('NO');
    ...
    $query = "SELECT id FROM " . NV_PREFIXLANG . "_" . $module_data . "_rows WHERE id!=" . $id . " AND catid=" . $catid . " ORDER BY weight ASC";
    $result = $db->query($query);
    $weight = 0;
    while ($row = $result->fetch()) {
        ++$weight;
    ...
    $sql = "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_rows SET weight=" . $new . " WHERE id=" . $id;
    $db->query($sql);
    die('OK');
}
...
if ($nv_Request->isset_request('changestatus', 'post')) {
    ...
    $query = "SELECT catid, status FROM " . NV_PREFIXLANG . "_" . $module_data . "_rows WHERE id=" . $id;
    $result = $db->query($query);
    $numrows = $result->rowCount();
    if ($numrows != 1) {
        die('NO');
    ...
    $sql = "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_rows SET status=" . $status . "_rows WHERE id=" . $id;
    $db->query($sql);
...
if ($nv_Request->isset_request('del', 'post')) {
    ...
    $sql = "SELECT COUNT(*) AS count, catid FROM " . NV_PREFIXLANG . "_" . $module_data . "_rows WHERE id=" . $id;
    $result = $db->query($sql);
...
$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM " . NV_PREFIXLANG . "_" . $module_data . "_rows";
