...
function nv_del_cat($catid)
{
    ...
    $sql = 'DELETE FROM ' . NV_PREFIXLANG . '_' . $module_data . '_rows WHERE catid=' . $catid;
    $db->query($sql);
...
if ($nv_Request->isset_request('add', 'get')) {
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        if (! $is_error) {
            $weight = $db->query('SELECT max(weight) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_cat WHERE parentid=' . intval($array['parentid']) . '')->fetchColumn();
...
if ($nv_Request->isset_request('edit', 'get')) {
    ...
    if ($nv_Request->isset_request('submit', 'post')) {
        ...
        if (empty($array['title'])) {
            ...
            if (! $is_error) {
                $sql = 'SELECT COUNT(*) AS count FROM ' . NV_PREFIXLANG . '_' . $module_data . '_cat WHERE id!=' . $catid . ' AND alias=' . $db->quote($array['alias']) . ' AND parentid=' . $array['parentid'];
                $result = $db->query($sql);
                $count = $result->fetchColumn();
        ...
        if (! $is_error) {
            if ($array['parentid'] != $row['parentid']) {
                $new_weight = $db->query('SELECT MAX(weight) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_cat WHERE parentid=' . $array['parentid'])->fetchColumn();
