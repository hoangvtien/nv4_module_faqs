...
if ( empty( $global_faqs_cat[$catid] ) ) die( 'Stop!!!' );
$page = 0;
if ( ! empty( $array_op[2] ) )
{
...
$base_url = "" . NV_BASE_SITEURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=" . $op . "/".$global_faqs_cat[$catid]['alias']."-".$catid;;
...
$top_contents = "";
if ( $global_faqs_cat[$catid]['parentid'] > 0 )
{
    $parentid_i = $global_faqs_cat[$catid]['parentid'];
    $array_cat_title = array();
    while ( $parentid_i > 0 )
    {
        $array_cat_title[] = $cur_link = "<a href=\"".$global_faqs_cat[$parentid_i]['link']."\">" . $global_faqs_cat[$parentid_i]['title'] . "</a>";
        $parentid_i = $global_faqs_cat[$parentid_i]['parentid'];
    }
    sort( $array_cat_title, SORT_NUMERIC );
    $top_contents = implode( " -> ", $array_cat_title );
}
$lik = ( empty($top_contents) )? "": " - ";
$cur_link = "<a href=\"".$global_faqs_cat[$catid]['link']."\">" . $global_faqs_cat[$catid]['title'] . "</a>";
$top_contents = "<div class=\"archives_links\">".$top_contents.$lik.$cur_link."</div>";

$html_pages = nv_faqs_page( $base_url, $all_page, $per_page, $page );
$contents = call_user_func( $global_faqs_cat[$catid]['viewcat'], $data_content, $top_contents ,$html_pages );
