...
global $global_faqs_cat;
$global_faqs_cat = array();
...
{
    $link_i = $link . "/" . $row['alias'] . "-" . $row['id'];
    $row['link'] = $link_i;
    $global_faqs_cat[$row['id']] = $row;
}
...
function nv_list_cats($is_link = false, $is_parentlink = true)
{
    ...
    $sql = "SELECT * FROM " . NV_PREFIXLANG . "_" . $module_data . "_cat WHERE status=1 ORDER BY parentid,weight ASC";
    $result = $db->query($sql);
...
function update_keywords($catid, $faq)
{
    ...
    if (! empty($keywords)) {
        $db->query("UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_cat SET keywords=" . $db->quote($keywords) . " WHERE id=" . $catid);
    }
